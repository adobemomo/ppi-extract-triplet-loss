...
class RNN_Relation(object):
    def __init__(self, num_classes, word_dict_size, d1_dict_size, d2_dict_size, type_dict_size, sentMax, wv,
        ...
        scores = self.rnn(X, 100, pooling, sentMax)
        ...
        self.loss = tf.reduce_mean(losses) + l2_reg_lambda * (tf.nn.l2_loss(self.W) + tf.nn.l2_loss(self.b))
    ...
    def rnn(self, X, num_filters, pooling, sentMax):
        # Recurrent Layer
        ...
        self.W = tf.Variable(tf.truncated_normal([2 * num_filters, self.num_classes], stddev=0.1), name="W")
        self.b = tf.Variable(tf.constant(0.1, shape=[self.num_classes]), name="b")

        scores = tf.nn.xw_plus_b(h2, self.W, self.b, name="scores")
    ...
    def train_step(self, W_batch, Sent_len, d1_batch, d2_batch, t_batch, y_batch, drop_out):
        ...
        _, step, loss, accuracy, predictions = \
            self.sess.run([self.train_op, self.global_step, self.loss, self.accuracy, self.predictions], feed_dict)
    ...
    def test_step(self, W_batch, Sent_len, d1_batch, d2_batch, t_batch, y_batch):
        ...
        step, loss, accuracy, predictions = \
            self.sess.run([self.global_step, self.loss, self.accuracy, self.predictions], feed_dict)
